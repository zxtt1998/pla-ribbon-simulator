...
async function exportCanvas(target,filename){if(!window.html2canvas){alert('导出组件尚未加载完成，请稍后再试。');return}const button=target===rack?$('#exportRack'):$('#exportUniform');const original=button.textContent;button.disabled=true;button.textContent='正在生成…';try{await document.fonts?.ready;const canvas=await window.html2canvas(target,{backgroundColor:target===rack?'#b8b9b5':null,scale:3,useCORS:true,logging:false});const link=document.createElement('a');link.download=filename;link.href=canvas.toDataURL('image/png');link.click()}catch(_){alert('导出失败，请确认图片已加载完成后重试。')}finally{button.disabled=false;button.textContent=original}}
$('#exportRack').addEventListener('click',()=>exportCanvas(rack,'勋表排列.png'));$('#exportUniform').addEventListener('click',()=>exportCanvas($('.uniform-canvas'),'勋表佩戴效果.png'));
render();
