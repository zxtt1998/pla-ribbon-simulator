...
const thumbnail = name => `assets/thumbnails/${name}.png`;
const ribbons = [
...
let selected=[], filter='all', query='', profile='officer-command', force='army', dragId=null;
...
const layoutSpacer=key=>({id:`layout-spacer-${key}`,layoutSpacer:true});
const putPositionInTopMiddle=items=>{const position=items.find(r=>r.category==='position');if(!position)return items;const rest=items.filter(r=>r.id!==position.id);const layout=[layoutSpacer('left'),position,layoutSpacer('right'),...rest];const padding=(3-layout.length%3)%3;return [...layout,...Array.from({length:padding},(_,i)=>layoutSpacer(`tail-${i}`))]};
const wearable=()=>{const sorted=ordered(),limited=sorted.length<=21?sorted:[...sorted.filter(r=>r.category==='position'||r.category==='term'),...sorted.filter(r=>r.category!=='position'&&r.category!=='term')].slice(0,21);return putPositionInTopMiddle(limited)};
const makeSelected=(r,extra={})=>({...r,awards:1,markerX:0,markerY:0,...extra});
const ribbonEl=(r,number='',useThumbnail=false)=>r.layoutSpacer?'<span class="layout-spacer" aria-hidden="true"></span>':`<button class="ribbon-with-awards ribbon-preview-trigger ${r.category==='position'?'position-ribbon':''}" type="button" data-preview="${r.id}" title="查看${r.name}大图"><span class="ribbon" ${number?`data-number="${number}"`:''}><img src="${useThumbnail?thumbnail(r.src.split('/').pop()):r.src}" alt="${r.name}勋表" loading="${useThumbnail?'lazy':'eager'}" decoding="async" ${useThumbnail?'fetchpriority="low"':''}/>${r.repeatable&&r.awards>1?`<b class="award-marker" data-marker-for="${r.id}" style="--marker-x:0px;--marker-y:0px" aria-label="获得${r.awards}次">${r.awards}</b>`:''}</span></button>`;
const awardOptions=r=>Array.from({length:r.maxAwards},(_,i)=>`<option value="${i+1}" ${r.awards===i+1?'selected':''}>${i+1}</option>`).join('');
function saveState(){try{localStorage.setItem(STORAGE_KEY,JSON.stringify({selected:selected.map(({id,awards,markerX,markerY})=>({id,awards,markerX,markerY})),filter,profile,force,enlistYear:$('#enlistYear')?.value||''}));}catch(_) {}}
function restoreState(){try{const saved=JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}');filter=saved.filter||filter;profile=saved.profile||profile;force=forceThemes[saved.force]?saved.force:force;selected=(saved.selected||[]).map(item=>{const source=ribbons.find(r=>r.id===item.id);return source?makeSelected(source,item):null}).filter(Boolean);return saved.enlistYear||''}catch(_){return ''}}
function renderCatalog(){const catalogTop=list.scrollTop,visible=ribbons.filter(r=>(filter==='all'||r.category===filter)&&(!r.profile||r.profile===profile)&&r.name.includes(query)),positionLocked=selected.some(x=>x.category==='position');list.innerHTML=visible.map(r=>{const isSelected=selected.some(x=>x.id===r.id),locked=r.category==='position'&&positionLocked&&!isSelected;return `<div class="catalog-item ${locked?'position-locked':''}">${ribbonEl(selected.find(x=>x.id===r.id)||r,'',true)}<div class="catalog-copy"><strong>${r.name}</strong><span>${r.kind} · ${r.subtitle}${r.repeatable?' · 可标次数':''}</span></div><button class="add-button ${isSelected?'selected':''}" data-add="${r.id}" ${locked?'disabled':''} aria-label="${isSelected?'移除':'添加'}${r.name}">${isSelected?'−':'+'}</button></div>`}).join('')||'<p class="empty-selected">当前身份下没有可选项目</p>';list.scrollTop=catalogTop}
function render(){const pageTop=window.scrollY,correct=ordered(),ok=selected.every((r,i)=>r.id===correct[i]?.id),shown=wearable(),overflow=selected.length>21,rows=Math.max(1,Math.ceil(shown.length/3)),canvas=$('.uniform-canvas');canvas.dataset.force=force;$('#uniformLabel').textContent=`${forceThemes[force].label}礼服左胸 · 勋表佩戴区`;$('#chosenCount').textContent=overflow?`${selected.length} 项已选 · 展示 21 项`:`${selected.length} 项已选`;$('#rowInfo').textContent=`${rows} / 7 行`;canvas.style.setProperty('--rack-rows',rows);const remainder=shown.length%3; rack.innerHTML=shown.map((r,i)=>`${i===1&&remainder===1?'<span class="rack-spacer" aria-hidden="true" style="grid-column:5 / span 2"></span>':''}<span class="rack-item" style="grid-column:${i===0&&remainder===1?'3 / span 2':i===0&&remainder===2?'2 / span 2':'span 2'}">${ribbonEl(r,String(i+1).padStart(2,'0'))}</span>`).join('');$('#canvasEmpty').style.display=shown.length?'none':'block';selectedList.innerHTML=selected.length?selected.map((r,i)=>`<div class="selected-row" draggable="true" data-id="${r.id}"><span class="rank">${String(i+1).padStart(2,'0')}</span><span class="drag">⋮⋮</span>${ribbonEl(r)}<div><strong>${r.name}</strong><small>${r.kind} · ${r.subtitle}</small>${r.repeatable?`<label class="award-control">获得次数 <select data-awards="${r.id}" aria-label="${r.name}获得次数">${awardOptions(r)}</select> 次</label>`:''}</div><button class="move" data-up="${r.id}" title="前移">↑</button><button class="delete" data-remove="${r.id}" title="移除">×</button></div>`).join(''):'<p class="empty-selected">尚未选择勋表</p>';const status=$('#ruleStatus');if(!selected.length){status.innerHTML='<span class="status-icon">✓</span><div><strong>等待选择勋表</strong><p>添加项目后将自动核验</p></div>';status.style.background='#ecf5ef'}else if(overflow){status.innerHTML='<span class="status-icon">!</span><div><strong>超过 21 枚佩戴上限</strong><p>预览已优先保留岗位职务和服役年限类</p></div>';status.style.background='#fff1ef'}else{status.innerHTML=ok?'<span class="status-icon">✓</span><div><strong>排列符合 23 式规则</strong><p>类别、战时优先与等级顺序均已核验</p></div>':'<span class="status-icon">!</span><div><strong>当前列表顺序已调整</strong><p>佩戴预览始终按规则自动排列</p></div>';status.style.background=ok?'#ecf5ef':'#fff1ef'}renderCatalog();saveState();requestAnimationFrame(()=>window.scrollTo(0,pageTop))}
function autoArrange(){selected=ordered();render()}
list.addEventListener('click',e=>{const id=e.target.dataset.add;if(!id)return;const item=ribbons.find(r=>r.id===id);if(selected.some(r=>r.id===id))selected=selected.filter(r=>r.id!==id);else{if(item.category==='position')selected=selected.filter(r=>r.category!=='position');selected=[...selected,makeSelected(item)]}autoArrange()});
...
$('#searchInput').addEventListener('input',e=>{query=e.target.value.trim();renderCatalog()});$('#resetButton').addEventListener('click',clearAll);$('#clearAll').addEventListener('click',clearAll);
...
async function drawRackExport(ctx,bounds){const rackRect=rectIn(rack.getBoundingClientRect(),bounds);ctx.fillStyle='#b8b9b5';ctx.fillRect(rackRect.x,rackRect.y,rackRect.width,rackRect.height);ctx.strokeStyle='#777b78';ctx.lineWidth=2;ctx.strokeRect(rackRect.x+1,rackRect.y+1,rackRect.width-2,rackRect.height-2);const images=[...rack.querySelectorAll('.ribbon img')];await Promise.all(images.map(waitForImage));images.forEach(img=>{if(!img.naturalWidth)return;const rect=rectIn(img.getBoundingClientRect(),bounds);ctx.drawImage(img,rect.x,rect.y,rect.width,rect.height);ctx.strokeStyle='rgba(30,38,40,.45)';ctx.lineWidth=1;ctx.strokeRect(rect.x+.5,rect.y+.5,rect.width-1,rect.height-1)});[...rack.querySelectorAll('.award-marker')].forEach(marker=>{const rect=rectIn(marker.getBoundingClientRect(),bounds);ctx.save();ctx.font=`700 ${Math.max(16,rect.height*.9)}px Georgia,serif`;ctx.textAlign='center';ctx.textBaseline='middle';ctx.lineWidth=2;ctx.strokeStyle='#4b443f';ctx.fillStyle='#f4f0e7';ctx.strokeText(marker.textContent,rect.x+rect.width/2,rect.y+rect.height/2);ctx.fillText(marker.textContent,rect.x+rect.width/2,rect.y+rect.height/2);ctx.restore()})}
const canvasBlob=canvas=>new Promise(resolve=>canvas.toBlob?canvas.toBlob(resolve,'image/png'):resolve(null));
async function exportCanvas(target,filename,exportTab){const button=target===rack?$('#exportRack'):$('#exportUniform'),original=button.textContent;button.disabled=true;button.textContent='正在生成…';try{await document.fonts?.ready;const bounds=target.getBoundingClientRect(),scale=2,canvas=document.createElement('canvas');canvas.width=Math.ceil(bounds.width*scale);canvas.height=Math.ceil(bounds.height*scale);const ctx=canvas.getContext('2d');ctx.scale(scale,scale);if(target===rack){ctx.fillStyle='#b8b9b5';ctx.fillRect(0,0,bounds.width,bounds.height)}else{ctx.fillStyle=forceThemes[force].background;ctx.fillRect(0,0,bounds.width,bounds.height);ctx.strokeStyle='rgba(255,255,255,.18)';ctx.lineWidth=1;ctx.beginPath();ctx.arc(0,-150,240,0,Math.PI*2);ctx.arc(bounds.width,-150,240,0,Math.PI*2);ctx.stroke();ctx.fillStyle='rgba(213,190,119,.85)';ctx.save();ctx.translate(-10,90);ctx.rotate(-.45);ctx.fillRect(-50,-25,190,60);ctx.translate(bounds.width+20,0);ctx.rotate(.9);ctx.fillRect(-50,-25,190,60);ctx.restore()}await drawRackExport(ctx,bounds);const blob=await canvasBlob(canvas);if(!blob)throw new Error('PNG 编码失败');const url=URL.createObjectURL(blob);if(exportTab){exportTab.location.href=url;setTimeout(()=>URL.revokeObjectURL(url),60000)}else{const link=document.createElement('a');link.download=filename;link.href=url;link.click();setTimeout(()=>URL.revokeObjectURL(url),60000)}}catch(_){exportTab?.close();alert('导出失败，请稍候重试。')}finally{button.disabled=false;button.textContent=original}}
function beginExport(target,filename){const exportTab=isMobileSafari()?window.open('about:blank','_blank'):null;if(exportTab)exportTab.document.write('<title>正在生成图片…</title><p style="font-family:system-ui;text-align:center;padding-top:20vh">正在生成 PNG，请稍候…</p>');exportCanvas(target,filename,exportTab)}
$('#exportRack').addEventListener('click',()=>beginExport(rack,'勋表排列.png'));$('#exportUniform').addEventListener('click',()=>beginExport($('.uniform-canvas'),'勋表佩戴效果.png'));
