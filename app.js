...
async function drawArmPatchExport(ctx,bounds){const item=armPatchById(armPatchId),patch=$('#armPatch');if(!item||patch.hidden)return;const image=await loadExportImage(armPatchSrc(item)),rect=rectIn(patch.getBoundingClientRect(),bounds);if(image)ctx.drawImage(image,rect.x,rect.y,rect.width,rect.height)}
const drawRackExportBase=drawRackExport;
drawRackExport=async(ctx,bounds,withDetails=false)=>{await drawRackExportBase(ctx,bounds,withDetails);if(bounds.width>rack.getBoundingClientRect().width+10)await drawArmPatchExport(ctx,bounds)};
