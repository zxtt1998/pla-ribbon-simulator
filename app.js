...
let selected=[], filter='all', query='', profile='officer-command', force='army', dragId=null, selectionSequence=0, repeatInputTimer=null, nameZh='姓名', namePinyin='XING MING', rankGroup='officer', rankId='', armPatchId='';
...
const rankInsignia=[
  ['officer-general','上将','officer','officer-general'],['officer-lieutenant-general','中将','officer','officer-lieutenant-general'],['officer-major-general','少将','officer','officer-major-general'],['officer-senior-colonel','大校','officer','officer-senior-colonel'],['officer-colonel','上校','officer','officer-colonel'],['officer-lieutenant-colonel','中校','officer','officer-lieutenant-colonel'],['officer-major','少校','officer','officer-major'],['officer-captain','上尉','officer','officer-captain'],['officer-first-lieutenant','中尉','officer','officer-first-lieutenant'],['officer-second-lieutenant','少尉','officer','officer-second-lieutenant'],
  ['nco-master-sergeant-1','一级军士长','nco','nco-master-sergeant-1'],['nco-master-sergeant-2','二级军士长','nco','nco-master-sergeant-2'],['nco-master-sergeant-3','三级军士长','nco','nco-master-sergeant-3'],['nco-staff-sergeant-2','二级上士','nco','nco-staff-sergeant-2'],['nco-staff-sergeant-1','一级上士','nco','nco-staff-sergeant-1'],['nco-sergeant','中士','nco','nco-sergeant'],['nco-corporal','下士','nco','nco-corporal']
].map(([id,name,group,asset])=>({id,name,group,asset}));
const rankById=id=>rankInsignia.find(rank=>rank.id===id);
const armPatchFiles=['中华人民共和国国防部.png','中国人民武装警察部队.png','中国人民解放军东部战区.svg','中国人民解放军中部战区.svg','中国人民解放军信息支援部队.png','中国人民解放军军事科学院.png','中国人民解放军军事航天部队.png','中国人民解放军北部战区.svg','中国人民解放军南部战区.svg','中国人民解放军国防大学.png','中国人民解放军国防科技大学.png','中国人民解放军战略支援部队.png','中国人民解放军海军.png','中国人民解放军火箭军.png','中国人民解放军空军.png','中国人民解放军网络空间部队.png','中国人民解放军联勤保障部队.png','中国人民解放军西部战区.svg','中国人民解放军陆军.png','中国人民解放军预备役部队.png','中国人民解放军驻澳门部队.png','中国人民解放军驻香港部队.png','中国人民解放军驻香港部队海军.png','中国人民解放军驻香港部队空军.png','中央军委.png','中央军委办公厅.png','中央军委后勤保障部.png','中央军委国防动员部.png','中央军委国际军事合作办公室.png','中央军委审计署.png','中央军委战略规划办公室.png','中央军委改革和编制办公室.png','中央军委政治工作部.png','中央军委政法委员会.png','中央军委机关事务管理总局.png','中央军委科学技术委员会.png','中央军委纪律检查委员会.png','中央军委联合参谋部.png','中央军委装备发展部.png','中央军委训练管理部.png'];
const armPatches=armPatchFiles.map(file=>({id:file,name:file.replace(/\.(png|svg)$/i,''),file}));
const armPatchById=id=>armPatches.find(item=>item.id===id);
const armPatchSrc=item=>`assets/arm-patches/${encodeURIComponent(item.file)}`;
// 火箭军、武警部队暂按陆军肩章颜色显示；海军、空军随预览军种切换。
...
function saveState(){try{localStorage.setItem(STORAGE_KEY,JSON.stringify({selected:selected.map(({id,selectionId,awards,markerX,markerY})=>({id,selectionId,awards,markerX,markerY})),filter,profile,force,nameZh,namePinyin,rankGroup,rankId,armPatchId,enlistYear:$('#enlistYear')?.value||''}));}catch(_) {}}
function restoreState(){try{const saved=JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}');filter=saved.filter||filter;profile=saved.profile||profile;force=forceThemes[saved.force]?saved.force:force;nameZh=(saved.nameZh||nameZh).trim().slice(0,6)||'姓名';namePinyin=(saved.namePinyin||namePinyin).trim().toUpperCase().slice(0,24)||'XING MING';rankGroup=['officer','nco'].includes(saved.rankGroup)?saved.rankGroup:rankGroup;rankId=rankById(saved.rankId)?.id||'';armPatchId=armPatchById(saved.armPatchId)?.id||'';selected=(saved.selected||[]).map(item=>{const source=ribbons.find(r=>r.id===item.id);return source?makeSelected(source,item):null}).filter(Boolean).filter(r=>!r.profile||r.profile===profile);return saved.enlistYear||''}catch(_){return ''}}
function renderRankPicker(){const select=$('#rankSelect'),groupRanks=rankInsignia.filter(rank=>rank.group===rankGroup);select.innerHTML=`<option value="">不显示军衔</option>${groupRanks.map(rank=>`<option value="${rank.id}">${rank.name}</option>`).join('')}`;if(rankById(rankId)?.group!==rankGroup)rankId='';select.value=rankId;document.querySelectorAll('#rankGroupControls button').forEach(button=>button.classList.toggle('active',button.dataset.rankGroup===rankGroup));}
function renderArmPatchPicker(){const select=$('#armPatchSelect');select.innerHTML=`<option value="">不显示臂章</option>${armPatches.map(item=>`<option value="${item.id}">${item.name}</option>`).join('')}`;select.value=armPatchId;}
...
function render(){
  const correct=ordered(),ok=selected.every((r,i)=>r.selectionId===correct[i]?.selectionId),shown=wearable(),maximum=21,overflow=selected.length>maximum,rows=Math.max(1,Math.ceil(shown.reduce((sum,r)=>sum+(r.gridSpan||2),0)/6)),canvas=$('.uniform-canvas'),chosenRank=rankById(rankId),rankPlate=$('#rankPlate'),chosenArmPatch=armPatchById(armPatchId),armPatch=$('#armPatch');
  canvas.dataset.force=force;
  $('#uniformLabel').textContent=`${forceThemes[force].label}礼服左胸 · 勋表佩戴区`;
  $('#namePlateZh').textContent=nameZh;$('#namePlatePinyin').textContent=namePinyin;
  rankPlate.hidden=!chosenRank;
  if(chosenRank){const rankSrc=rankImageSrc(chosenRank),rankAlt=`${rankForceLabel()}${chosenRank.name}肩章`;$('#rankInsigniaLeft').src=rankSrc;$('#rankInsigniaRight').src=rankSrc;$('#rankInsigniaLeft').alt=rankAlt;$('#rankInsigniaRight').alt=rankAlt;$('#rankPlateLabel').textContent=chosenRank.name}
  armPatch.hidden=!chosenArmPatch;
  if(chosenArmPatch){armPatch.src=armPatchSrc(chosenArmPatch);armPatch.alt=`${chosenArmPatch.name}臂章`}
  $('#chosenCount').textContent=overflow?`${selected.length} 项已选 · 展示 ${maximum} 项`:`${selected.length} 项已选`;
  $('#rowInfo').textContent=`${rows} / 7 行`;canvas.style.setProperty('--rack-rows',rows);
  rack.innerHTML=shown.map((r,i)=>`<span class="rack-item" style="grid-column:span ${r.gridSpan||2}">${ribbonEl(r,String(i+1).padStart(2,'0'),true)}</span>`).join('');
  $('#ribbonAssembly').style.display=shown.length?'block':'none';$('#canvasEmpty').style.display=shown.length?'none':'block';
  selectedList.innerHTML=selected.length?selected.map((r,i)=>`<div class="selected-row" draggable="true" data-id="${r.selectionId}"><span class="rank">${String(i+1).padStart(2,'0')}</span><span class="drag">⋮⋮</span>${ribbonEl(r,'',true)}<div><strong>${r.name}</strong><small>${r.kind} · ${r.subtitle}</small>${r.repeatable?`<label class="award-control">获得次数 <select data-awards="${r.selectionId}" aria-label="${r.name}获得次数">${awardOptions(r)}</select> 次</label>`:''}</div><button class="move" data-up="${r.selectionId}" title="前移">↑</button><button class="delete" data-remove="${r.selectionId}" title="移除" ${r.category==='term'?'disabled':''}>×</button></div>`).join(''):'<p class="empty-selected">尚未选择勋表</p>';
  const status=$('#ruleStatus');
  if(!selected.length){status.innerHTML='<span class="status-icon">✓</span><div><strong>等待选择勋表</strong><p>添加项目后将自动核验</p></div>';status.style.background='#ecf5ef'}else if(overflow){status.innerHTML='<span class="status-icon">!</span><div><strong>超过可佩戴上限</strong><p>预览已按七排上限保留优先项目</p></div>';status.style.background='#fff1ef'}else{status.innerHTML=ok?'<span class="status-icon">✓</span><div><strong>排列符合 23 式规则</strong><p>类别、战时优先与等级顺序均已核验</p></div>':'<span class="status-icon">!</span><div><strong>当前列表顺序已调整</strong><p>佩戴预览始终按规则自动排列</p></div>';status.style.background=ok?'#ecf5ef':'#fff1ef'}
  renderCatalog();saveState()
}
...
$('#identityControls').addEventListener('click',e=>{if(!e.target.dataset.profile)return;profile=e.target.dataset.profile;selected=selected.filter(r=>!r.profile||r.profile===profile);document.querySelectorAll('#identityControls button').forEach(b=>b.classList.toggle('active',b===e.target));const expectedGroup=profile==='nco'?'nco':'officer';if(rankGroup!==expectedGroup){rankGroup=expectedGroup;rankId='';renderRankPicker()}autoArrange()});
...
renderRankPicker();
renderArmPatchPicker();
$('#rankGroupControls').addEventListener('click',e=>{const next=e.target.dataset.rankGroup;if(!next||next===rankGroup)return;rankGroup=next;rankId='';renderRankPicker();render()});
$('#rankSelect').addEventListener('change',e=>{rankId=rankById(e.target.value)?.id||'';render()});
$('#armPatchSelect').addEventListener('change',e=>{armPatchId=armPatchById(e.target.value)?.id||'';render()});
...
const clearAll=()=>{selected=[];enlistYear.value='';armPatchId='';nameZh='姓名';namePinyin='XING MING';$('#nameInput').value=nameZh;$('#pinyinInput').value=namePinyin;$('#armPatchSelect').value='';render()};
...
async function drawRankPlateExport(ctx,bounds){const selectedRank=rankById(rankId),plate=$('#rankPlate');if(!selectedRank||plate.hidden)return;const rect=rectIn(plate.getBoundingClientRect(),bounds),left=rectIn($('#rankInsigniaLeft').getBoundingClientRect(),bounds),right=rectIn($('#rankInsigniaRight').getBoundingClientRect(),bounds),source=await loadExportImage(rankImageSrc(selectedRank)),drawHorizontalRank=(target,angle)=>{ctx.save();ctx.translate(target.x+target.width/2,target.y+target.height/2);ctx.rotate(angle);ctx.drawImage(source,-target.height/2,-target.width/2,target.height,target.width);ctx.restore()};if(!source)return;drawHorizontalRank(left,Math.PI/2);drawHorizontalRank(right,-Math.PI/2);ctx.fillStyle='#f6f2e7';ctx.textAlign='center';ctx.textBaseline='middle';ctx.font=`700 ${Math.max(8,rect.height*.28)}px serif`;ctx.fillText(selectedRank.name,rect.x+rect.width/2,rect.y+rect.height/2);ctx.textAlign='start'}
function drawNamePlateExport(ctx,bounds){const plate=rectIn($('#namePlate').getBoundingClientRect(),bounds);ctx.fillStyle='#172d49';ctx.fillRect(plate.x,plate.y,plate.width,plate.height);ctx.strokeStyle='#e2e6e7';ctx.lineWidth=2;ctx.strokeRect(plate.x+1,plate.y+1,plate.width-2,plate.height-2);ctx.fillStyle='#fff';ctx.textAlign='center';ctx.textBaseline='middle';ctx.font=`${Math.max(13,plate.height*.37)}px ${NAMEPLATE_ZH_FONT}`;ctx.fillText(nameZh,plate.x+plate.width/2,plate.y+plate.height*.38);ctx.font=`700 ${Math.max(8,plate.height*.19)}px sans-serif`;ctx.fillText(namePinyin,plate.x+plate.width/2,plate.y+plate.height*.73);ctx.textAlign='start'}
async function drawRackExport(ctx,bounds,withDetails=false){await drawRankPlateExport(ctx,bounds);drawNamePlateExport(ctx,bounds);const rackRect=rectIn(rack.getBoundingClientRect(),bounds);ctx.fillStyle='#b8b9b5';ctx.fillRect(rackRect.x,rackRect.y,rackRect.width,rackRect.height);ctx.strokeStyle='#777b78';ctx.lineWidth=2;ctx.strokeRect(rackRect.x+1,rackRect.y+1,rackRect.width-2,rackRect.height-2);const entries=[...rack.querySelectorAll('.ribbon img')].map(img=>({img,item:ribbons.find(r=>r.id===img.closest('[data-preview]')?.dataset.preview),rect:rectIn(img.getBoundingClientRect(),bounds)})).filter(entry=>entry.item),sources=new Map;await Promise.all([...new Set(entries.map(entry=>entry.item.src))].map(async src=>sources.set(src,await loadExportImage(src))));entries.forEach(({img,item,rect})=>{const source=sources.get(item.src)||img;if(!source?.naturalWidth)return;ctx.drawImage(source,rect.x,rect.y,rect.width,rect.height);ctx.strokeStyle='rgba(30,38,40,.45)';ctx.lineWidth=1;ctx.strokeRect(rect.x+.5,rect.y+.5,rect.width-1,rect.height-1)});[...rack.querySelectorAll('.award-marker')].forEach(marker=>{const rect=rectIn(marker.getBoundingClientRect(),bounds);ctx.save();ctx.font=`700 ${Math.max(16,rect.height*.9)}px Georgia,serif`;ctx.textAlign='center';ctx.textBaseline='middle';ctx.lineWidth=2;ctx.strokeStyle='#4b443f';ctx.fillStyle='#f4f0e7';ctx.strokeText(marker.textContent,rect.x+rect.width/2,rect.y+rect.height/2);ctx.fillText(marker.textContent,rect.x+rect.width/2,rect.y+rect.height/2);ctx.restore()});if(withDetails)drawRackDetails(ctx,bounds,entries)}
async function drawArmPatchExport(ctx,bounds,isRackExport=false){const item=armPatchById(armPatchId),patch=$('#armPatch');if(!item||patch.hidden)return;const image=await loadExportImage(armPatchSrc(item));if(!image)return;const rect=isRackExport?{x:bounds.width+12,y:10,width:58,height:76}:rectIn(patch.getBoundingClientRect(),bounds);ctx.drawImage(image,rect.x,rect.y,rect.width,rect.height)}
const drawRackExportBase=drawRackExport;
drawRackExport=async(ctx,bounds,withDetails=false,isRackExport=false)=>{await drawRackExportBase(ctx,bounds,withDetails);await drawArmPatchExport(ctx,bounds,isRackExport)};
const canvasBlob=canvas=>new Promise(resolve=>canvas.toBlob?canvas.toBlob(resolve,'image/png'):resolve(null));
async function exportCanvas(target,filename,exportTab,withDetails=false){const isRackExport=target===$('#ribbonAssembly'),button=withDetails?$('#exportRackDetails'):isRackExport?$('#exportRack'):$('#exportUniform'),original=button.textContent;button.disabled=true;button.textContent='正在生成…';try{await document.fonts?.ready;const bounds=target.getBoundingClientRect(),scale=4,extraHeight=withDetails?detailHeight():0,patchMargin=isRackExport&&armPatchById(armPatchId)?82:0,canvas=document.createElement('canvas');canvas.width=Math.ceil((bounds.width+patchMargin)*scale);canvas.height=Math.ceil((bounds.height+extraHeight)*scale);const ctx=canvas.getContext('2d');ctx.scale(scale,scale);if(isRackExport){ctx.fillStyle='#fff';ctx.fillRect(0,0,bounds.width+patchMargin,bounds.height+extraHeight);ctx.fillStyle='#b8b9b5';ctx.fillRect(0,0,bounds.width,bounds.height)}else{ctx.fillStyle=forceThemes[force].background;ctx.fillRect(0,0,bounds.width,bounds.height);ctx.strokeStyle='rgba(255,255,255,.18)';ctx.lineWidth=1;ctx.beginPath();ctx.arc(0,-150,240,0,Math.PI*2);ctx.arc(bounds.width,-150,240,0,Math.PI*2);ctx.stroke();ctx.fillStyle='rgba(213,190,119,.85)';ctx.save();ctx.translate(-10,90);ctx.rotate(-.45);ctx.fillRect(-50,-25,190,60);ctx.translate(bounds.width+20,0);ctx.rotate(.9);ctx.fillRect(-50,-25,190,60);ctx.restore()}await drawRackExport(ctx,bounds,withDetails,isRackExport);const blob=await canvasBlob(canvas);if(!blob)throw new Error('PNG 编码失败');const url=URL.createObjectURL(blob);if(exportTab){exportTab.location.href=url;setTimeout(()=>URL.revokeObjectURL(url),60000)}else{const link=document.createElement('a');link.download=filename;link.href=url;link.click();setTimeout(()=>URL.revokeObjectURL(url),60000)}}catch(_){exportTab?.close();alert('导出失败，请稍候重试。')}finally{button.disabled=false;button.textContent=original}}
