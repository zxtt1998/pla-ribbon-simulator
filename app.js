...
const putPositionInTopMiddle=items=>{const position=items.find(r=>r.category==='position');if(!position)return items;const rest=items.filter(r=>r.id!==position.id);return [rest[0],position,rest[1],...rest.slice(2)].filter(Boolean)};
...
const ribbonEl=(r,number='')=>`<button class="ribbon-with-awards ribbon-preview-trigger ${r.category==='position'?'position-ribbon':''}" type="button" data-preview="${r.id}" title="查看${r.name}大图"><span class="ribbon" ${number?`data-number="${number}"`:''}><img src="${r.src}" alt="${r.name}勋表" />${r.repeatable&&r.awards>1?`<b class="award-marker" data-marker-for="${r.id}" style="--marker-x:${r.markerX||0}px;--marker-y:${r.markerY||0}px" aria-label="获得${r.awards}次">${r.awards}</b>`:''}</span></button>`;
const awardOptions=r=>Array.from({length:r.maxAwards},(_,i)=>`<option value="${i+1}" ${r.awards===i+1?'selected':''}>${i+1}</option>`).join('');
const markerControls=r=>`<div class="marker-controls"><span>数字位置</span><label>横 <input type="range" min="-28" max="28" value="${r.markerX||0}" data-marker-x="${r.id}" aria-label="${r.name}数字横向位置" /></label><label>纵 <input type="range" min="-22" max="22" value="${r.markerY||0}" data-marker-y="${r.id}" aria-label="${r.name}数字纵向位置" /></label><button type="button" data-marker-reset="${r.id}">恢复默认</button></div>`;
function saveState(){try{localStorage.setItem(STORAGE_KEY,JSON.stringify({selected:selected.map(({id,awards,markerX,markerY})=>({id,awards,markerX,markerY})),filter,profile,enlistYear:$('#enlistYear')?.value||''}));}catch(_) {}}
function restoreState(){try{const saved=JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}');filter=saved.filter||filter;profile=saved.profile||profile;selected=(saved.selected||[]).map(item=>{const source=ribbons.find(r=>r.id===item.id);return source?makeSelected(source,item):null}).filter(Boolean);return saved.enlistYear||''}catch(_){return ''}}
...
selectedList.addEventListener('input',e=>{const id=e.target.dataset.markerX||e.target.dataset.markerY;if(!id)return;const key=e.target.dataset.markerX?'markerX':'markerY',value=Number(e.target.value);selected=selected.map(r=>r.id===id?{...r,[key]:value}:r);document.querySelectorAll(`[data-marker-for="${id}"]`).forEach(marker=>marker.style.setProperty(key==='markerX'?'--marker-x':'--marker-y',`${value}px`));saveState()});
...
enlistYear.addEventListener('change',e=>{const year=Number(e.target.value);if(!year){render();return}const years=currentYear-year,terms=termForYears(years),termItems=terms.map(value=>ribbons.find(r=>r.category==='term'&&r.name===`${termLabel[value]}年军龄略章`)).filter(Boolean);selected=[...selected.filter(r=>r.category!=='term'),...termItems.map(r=>makeSelected(r))];render()});
$('#searchInput').addEventListener('input',e=>{query=e.target.value.trim();renderCatalog()});$('#autoArrange').addEventListener('click',autoArrange);$('#resetButton').addEventListener('click',()=>{selected=[];enlistYear.value='';render()});
...
async function exportCanvas(target,filename,exportTab){if(!window.html2canvas){exportTab?.close();alert('导出组件尚未加载完成，请稍后再试。');return}const button=target===rack?$('#exportRack'):$('#exportUniform');const original=button.textContent;button.disabled=true;button.textContent='正在生成…';try{await document.fonts?.ready;const canvas=await window.html2canvas(target,{backgroundColor:target===rack?'#b8b9b5':null,scale:3,useCORS:true,logging:false});const dataUrl=canvas.toDataURL('image/png');if(exportTab){exportTab.location.href=dataUrl}else{const link=document.createElement('a');link.download=filename;link.href=dataUrl;link.click()}}catch(_){exportTab?.close();alert('导出失败，请确认图片已加载完成后重试。')}finally{button.disabled=false;button.textContent=original}}
