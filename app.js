...
const ribbonEl=(r,number='',thumbnail=false)=>r.layoutSpacer?'<span class="layout-spacer" aria-hidden="true"></span>':`<button class="ribbon-with-awards ribbon-preview-trigger ${r.category==='position'?'position-ribbon':''}" type="button" data-preview="${r.id}" title="查看${r.name}大图"><span class="ribbon" ${number?`data-number="${number}"`:''}><img src="${r.src}" alt="${r.name}勋表" loading="${thumbnail?'lazy':'eager'}" decoding="async" ${thumbnail?'fetchpriority="low"':''}/>${r.repeatable&&r.awards>1?`<b class="award-marker" data-marker-for="${r.id}" style="--marker-x:0px;--marker-y:0px" aria-label="获得${r.awards}次">${r.awards}</b>`:''}</span></button>`;
