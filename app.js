...
const centerLastRow=items=>{const count=items.length%3;if(!count)return items;const complete=items.slice(0,-count),last=items.slice(-count);return count===1?[...complete,layoutSpacer('last-left'),last[0],layoutSpacer('last-right')]:[...complete,layoutSpacer('last-left',1),...last,layoutSpacer('last-right',1)]};
const putPositionInTopMiddle=items=>{const position=items.find(r=>r.category==='position');if(!position)return centerFirstRow(items);const rest=items.filter(r=>r.id!==position.id);return [layoutSpacer('position-left'),position,layoutSpacer('position-right'),...centerLastRow(rest)]};
const wearable=()=>{const sorted=ordered(),hasPosition=sorted.some(r=>r.category==='position'),maximum=hasPosition?19:21,limited=sorted.length<=maximum?sorted:[...sorted.filter(r=>r.category==='position'||r.category==='term'),...sorted.filter(r=>r.category!=='position'&&r.category!=='term')].slice(0,maximum);return putPositionInTopMiddle(limited)};
const makeSelected=(r,extra={})=>({...r,awards:1,markerX:0,markerY:0,...extra});
const ribbonEl=(r,number='',useThumbnail=false)=>r.layoutSpacer?'<span class="layout-spacer" aria-hidden="true"></span>':`<button class="ribbon-with-awards ribbon-preview-trigger ${r.category==='position'?'position-ribbon':''}" type="button" data-preview="${r.id}" title="查看${r.name}大图"><span class="ribbon" ${number?`data-number="${number}"`:''}><img src="${useThumbnail?thumbnail(r.src.split('/').pop()):r.src}" alt="${r.name}勋表" loading="eager" decoding="async"/>${r.repeatable&&r.awards>1?`<b class="award-marker" data-marker-for="${r.id}" style="--marker-x:0px;--marker-y:0px" aria-label="获得${r.awards}次">${r.awards}</b>`:''}</span></button>`;
