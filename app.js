...
async function drawRankPlateExport(ctx,bounds){const selectedRank=rankById(rankId),plate=$('#rankPlate');if(!selectedRank||plate.hidden)return;const rect=rectIn(plate.getBoundingClientRect(),bounds),left=rectIn($('#rankInsigniaLeft').getBoundingClientRect(),bounds),right=rectIn($('#rankInsigniaRight').getBoundingClientRect(),bounds),source=await loadExportImage(selectedRank.src);if(!source)return;ctx.drawImage(source,left.x,left.y,left.width,left.height);ctx.save();ctx.translate(right.x+right.width,right.y);ctx.scale(-1,1);ctx.drawImage(source,0,0,right.width,right.height);ctx.restore();ctx.fillStyle='#f6f2e7';ctx.textAlign='center';ctx.textBaseline='middle';ctx.font=`700 ${Math.max(9,rect.height*.17)}px serif`;ctx.fillText(selectedRank.name,rect.x+rect.width/2,rect.y+rect.height/2);ctx.textAlign='start'}
