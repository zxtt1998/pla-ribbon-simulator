...
let selected=[], filter='all', query='', profile='officer-command', force='army', dragId=null, selectionSequence=0;
...
function saveState(){try{localStorage.setItem(STORAGE_KEY,JSON.stringify({selected:selected.map(({id,selectionId,awards,markerX,markerY})=>({id,selectionId,awards,markerX,markerY})),filter,profile,force,enlistYear:$('#enlistYear')?.value||''}));}catch(_) {}}
...
$('#exportRack').addEventListener('click',()=>beginExport(rack,'勋表排列-原图.png'));$('#exportRackDetails').addEventListener('click',()=>beginExport(rack,'勋表排列-含说明.png',true));$('#exportUniform').addEventListener('click',()=>beginExport($('.uniform-canvas'),'勋表佩戴效果-原图.png'));
